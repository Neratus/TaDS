...
void delete_words_starting_with(struct tree_node_t **root, char letter) 
{
    ...
    {
        *root = delete_node(*root, (*root)->name);
...
void in_order_silent(struct tree_node_t *node, int *cmp) 
{
    if (node != NULL) 
    {
        if (node->height > *cmp)
            *cmp = node->height;
        in_order_silent(node->left, cmp);
        in_order_silent(node->right, cmp);
    }
}
...
struct tree_node_t *delete_node(struct tree_node_t *tree, const char *name)
{
    if (tree == NULL)
        return tree;
    ...
    else 
    {
        ...
        tree->right = delete_node(tree->right, temp->name);
    }
    tree->height = 1 + max(height(tree->left), height(tree->right));
